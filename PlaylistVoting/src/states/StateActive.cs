...
public class StateActive : State
{
    ...
    {
        List<OnlineZeeplevel> newPlaylist = new List<OnlineZeeplevel>();
        foreach (OnlineZeeplevel level in ZeepkistNetwork.CurrentLobby.Playlist)
        {
            if (!level.UID.Equals(Plugin.Instance.uid))
            {
                newPlaylist.Add(level);
            }
        }
    ...
    {
        int delay = 2;
        if (noVotes >= yesVotes && Plugin.Instance.DeleteNoLevels && ZeepkistNetwork.CurrentLobby.Playlist.Count > 1)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        yesVotes = int.Parse(match.Groups[1].Value);
                        noVotes = int.Parse(match.Groups[2].Value);
                        ...
                        {
                            ChatApi.SendMessage(
                                $"/servermessage white 0 <align=\"left\"><size=\"30%\"><br><br>{message}<br><#ffffff></size><size=\"20%\"><voffset=-0.5em>Type !y in the chat if you like the current level</voffset><br>Type !n in the chat if not");
                        }
                    }
                    else
                    {
